...
#define OFF_HD  0x58EB54u // Target: CHud::Draw
...
static void hook_HD(){
    // Gambar HUD asli gamenya dulu
    if(gOHD) ((fn_HD)gOHD)();
    // Tambahkan watermark kita
    if(g_ready) draw();
}
...
static void* init_thread(void*) {
    uintptr_t b = 0;
    // Tunggu libGTASA termuat (menggunakan proc/maps POSIX)
    while (!(b = get_base_posix("libGTASA.so"))) { sleep(1); }
    
    // DELAY 5 DETIK! 
    // Sangat penting agar layar loading server SAMP selesai dan game stabil
    sleep(5); 
...
# ── 1. Buat ulang main.cpp (Metode PROC + Sleep 5 Detik + Hardcode) ──
...
# ── 1. Update main.cpp (Fix Offset CHud::Draw v1.08) ──
cat > jni/main.cpp << 'EOF'
#include <jni.h>
#include <android/log.h>
#include <sys/mman.h>
...
// OFFSET GTA SA v1.08 (Sesuai ReadELF kamu)
#define OFF_PS  0x583D71u
#define OFF_SC  0x582D01u
#define OFF_SS  0x582C9Du
#define OFF_SO  0x582CE5u
...
#define OFF_SF  0x582C65u
#define OFF_SE  0x582D8Du
#define OFF_HD  0x43A3FDu // FIX: Ini CHud::Draw untuk v1.08!
...
static void* init_thread(void*) {
    uintptr_t b = 0;
    while (!(b = get_base_posix("libGTASA.so"))) { sleep(1); }
    sleep(5); // Delay wajib agar tidak crash saat loading server
    ...
    if(thumb_hook(b+(OFF_HD&~1u),(void*)hook_HD,(void**)&gOHD)){
        g_ready = true; 
        LOGI("Watermark 'Riski Aja' siap digambar!");
    }
    return nullptr;
}
